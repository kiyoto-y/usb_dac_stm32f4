...
#define AUDIO_FEEDBACK_EP                             0x81
#define USB_AUDIO_CONFIG_DESC_SIZ                     (109 + 1 + 9/* 追加分 */)
...
#define AUDIO_CONTROL_SAMPLING_FREQ                   (0x0001)
...
/* @memo Windows標準Drvでは、再生可能な最大ｻﾝﾌﾟﾘﾝｸﾞﾚｰﾄ+10%よりも大きいと認識されないっぽい? */
#define AUDIO_OUT_PACKET                              (((USBD_AUDIO_FREQ * 4 * 2/* bytes /s */) * 11 / 10/* +10% */) / 1000/* bytes/s -> bytes/ms */)
#define AUDIO_DEFAULT_VOLUME                          (0x0000)	/* -0.0dB */
...
typedef enum
{
  ...
  AUDIO_CMD_PLAY,
  AUDIO_CMD_STOP,
  AUDIO_CMD_MISSING,	/* 所定のﾃﾞｰﾀが欠けた場合(incomplete割り込み時) */
}AUDIO_CMD_TypeDef;
...
typedef struct
{
    int8_t  (*Init)         (uint32_t AudioFreq, int16_t Volume, uint32_t options);
